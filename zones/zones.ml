...
let () =
  if Array.length Sys.argv < 5 then begin
    print_string "Missing arguments. Valid syntax: program (input) [NB_ZONES] [NB_ROLLS] (method) (output=classes.txt)\n";
    exit (-1)
  end;
  ...
  let output = if Array.length Sys.argv > 5 then Sys.argv.(5) else "classes.txt" in
  ...
  | (name, f) :: _ when name = meth ->
    ...
    let res = f points zones rolls in
